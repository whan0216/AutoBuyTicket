...
public class AtomLogic
{
    public static Header[] getHeaderCookie()
    {
        ...
        try
        {
            ...
            LogFactory.info("获取https cookie:" + java.util.Arrays.toString(httpsCookieHeaders));
        }
        catch (Exception e)
        {
            e.printStackTrace();
    ...
    public static Map<String, StationNameInfo> getStationNames()
    {
        ...
        try
        {
            ...
            String stationNames = CommonUtil.getContent(response.getEntity().getContent());
            ...
            // 缓存此次得到的文件
            CommonUtil.writeCacheFile(Constant.CACHE_STATION_NAMES, stationNames);
        }
        catch (Exception e)
        {
            e.printStackTrace();
     ...
     * @see [类、类#方法、类#成员]
     */
    public static Map<String, TrainNoInfo> getTrainNoInfo(TicketQryCondition qryCondition, Header[] cookieHeaders)
    {
        Map<String, TrainNoInfo> retMap = new HashMap<String, TrainNoInfo>();

        List<NameValuePair> nvps = CommonUtil.getParamList(qryCondition, "queryststrainall");

        Header[] headers = HttpUtil.getRequestHeader(Constant.URL_4_REFERER_REMANENT_TICKET, cookieHeaders);

        for (int idx = 0; idx < 5; idx++)
        {
            try
            {
                HttpResponse response = HttpUtil.sendPost(Constant.URL_4_STS_TRAIN_ALL, nvps, headers);
                String content = CommonUtil.getContent(response.getEntity().getContent());
     ...
     * @see [类、类#方法、类#成员]
     */
    public static Map<String, RemainTicketInfo> getRemanentTicket(String refererUrl, TicketQryCondition qryCondition,
            Header[] cookieHeaders)
    {
        Map<String, RemainTicketInfo> retmap = new HashMap<String, RemainTicketInfo>();

        String params = CommonUtil.getParamsString(qryCondition, "queryLeftTicket");

        String url = refererUrl.replace("init", "queryLeftTicket") + "&" + params;
            ...
            if (!StringUtil.isEmpty(datas) && datas.trim().startsWith("0"))
            {
                ...
                for (String data : dataArray)
                {
                    ...
                    else if (num == 1 && null != currInfo)
                    {
                        ...
                        currInfo.setFrom_station_name(data.substring(0, data.length() - 5));
                    ...
                    {
                        data = data.replaceAll("&nbsp;", "").trim();
                        currInfo.setTo_station_name(data.substring(0, data.length() - 5));
                    ...
                    else if (num == 15 && null != currInfo)
                    {
                        ...
                        if (isOrder && data.contains("getSelected"))
                        {
                            ...
                            currInfo.setSelectedInfo(new TrainSelectedInfo(currInfo.getTagInfo()));
                        }
    ...
    public static File getVerifyCode(String url, Header[] cookieHeaders)
    {
        ...
        if (null != cookieHeaders)
        {
            ...
            {
                // 获取登陆验证码
                Header[] headers = HttpUtil.getRequestHeader(Constant.URL_4_REFERER_LOGIN_IN, cookieHeaders);

                HttpResponse response = HttpUtil.sendGet(url, headers);
                InputStream in = response.getEntity().getContent();

                ret = new File(SysConfig.getDefConfig().getCache_path(), Constant.CACHE_VERIFY_CODE_IMG);
     ...
     * @see [类、类#方法、类#成员]
     */
    public static File getOrderPassCode(Header[] cookieHeaders) throws Exception
    {
        File ret = null;
        if (null != cookieHeaders)
        {
            ...
            {
                // 获取订单验证码
                Header[] headers = HttpUtil.getRequestHeader(Constant.URL_4_REFERER_PASSENGER, cookieHeaders);
                HttpResponse response = HttpUtil.sendGet(Constant.URL_4_PASS_CODE, headers);
                InputStream in = response.getEntity().getContent();

                ret = new File(SysConfig.getDefConfig().getCache_path(), Constant.CACHE_PASS_CODE_IMG);
    ...
    public static boolean login(LoginInfo loginInfo, Header[] cookieHeaders)
    {
        try
        {
            ...
            HttpResponse response = HttpUtil.sendPost(Constant.URL_4_RAND_CODE, (Map<String, String>) null, headers);
            String content = CommonUtil.getContent(response.getEntity().getContent());
            JSONObject jo = JSONObject.fromObject(content);
            ...
            String loginResult = CommonUtil.getContent(response.getEntity().getContent());
     ...
     * @see [类、类#方法、类#成员]
     */
    public static String[] doPreBookTicket(TicketQryCondition ticket, RemainTicketInfo remain, Header[] cookieHeaders)
    {
        String[] ret = null;
        Header[] headers = HttpUtil.getRequestHeader(Constant.URL_4_REFERER_ORDER, cookieHeaders);
        ...
        List<NameValuePair> nvps = CommonUtil.getParamList(ticket, "submutOrderRequest");
        nvps.addAll(CommonUtil.getParamList(remain.getSelectedInfo(), "submutOrderRequest"));

        try
        {
            HttpResponse response = HttpUtil.sendPost(Constant.URL_4_PRE_BOOK_TICKET, nvps, headers);
            String beforeOrderResult = CommonUtil.getContent(response.getEntity().getContent());
            ...
            if (statusCode == HttpStatus.SC_MOVED_PERMANENTLY || statusCode == HttpStatus.SC_MOVED_TEMPORARILY)
            {
                ...
                response = HttpUtil.sendPost(redirectUrl, (Map<String, String>) null, headers);
                String responseBody = CommonUtil.getContent(response.getEntity().getContent());
    ...
    public static boolean doSubmitOrder(String randCode, String[] tokens, Header[] cookieHeaders,
            List<PassengerInfo> passengers, TicketQryCondition ticket, RemainTicketInfo remain)
    {
        boolean ret = false;
        try
        {
            List<NameValuePair> nvps = CommonUtil.getParamList(ticket, "confirmSingleForQueueOrder");
            nvps.addAll(CommonUtil.getParamList(remain.getSelectedInfo(), "confirmSingleForQueueOrder"));

            nvps.add(new BasicNameValuePair("org.apache.struts.taglib.html.TOKEN", tokens[0]));
            nvps.add(new BasicNameValuePair("leftTicketStr", tokens[1]));
            nvps.add(new BasicNameValuePair("textfield", "中文或拼音首字母"));
            nvps.add(new BasicNameValuePair("orderRequest.seat_type_code", ""));
            nvps.add(new BasicNameValuePair("orderRequest.seat_detail_type_code", ""));
            nvps.add(new BasicNameValuePair("orderRequest.ticket_type_order_num", ""));
            nvps.add(new BasicNameValuePair("orderRequest.bed_level_order_num", "000000000000000000000000000000"));
            ...
            nvps.add(new BasicNameValuePair("tFlag", "dc"));
            ...
            if (checkOrderInfo(randCode, nvps, cookieHeaders))
            {
                for (PassengerInfo passenger : passengers)
                {
                    getQueueCount(passenger.getSeat_type(), ticket, remain, tokens, cookieHeaders);
                }

                Header[] headers = HttpUtil.getRequestHeader(Constant.URL_4_REFERER_PASSENGER, cookieHeaders);

                HttpResponse response = HttpUtil.sendPost(Constant.URL_4_CONFIRM_ORDER + "&rand=" + randCode, nvps,
                        headers);
                String result = CommonUtil.getContent(response.getEntity().getContent());
     ...
     * @return boolean
     */
    public static boolean checkOrderInfo(String rand, List<NameValuePair> nvps, Header[] cookieHeaders)
    {
        boolean ret = false;
        ...
        try
        {
            HttpResponse response = HttpUtil.sendPost(Constant.URL_4_CHECK_ORDER_INFO+ "&rand=" + rand, nvps, headers);
            String result = CommonUtil.getContent(response.getEntity().getContent());
     ...
     * @see [类、类#方法、类#成员]
     */
    public static void getQueueCount(String seat, TicketQryCondition ticket, RemainTicketInfo remain, String[] tokens,
            Header[] cookieHeaders)
    {
    ...
    public static List<PassengerInfo> getPassenger(Header[] cookieHeaders)
    {
        ...
        try
        {
            HttpResponse response = HttpUtil
                    .sendPost(Constant.URL_4_GET_PASSENGER, (Map<String, String>) null, headers);
            String content = CommonUtil.getContent(response.getEntity().getContent());
    ...
    public static void main(String[] args) throws IOException
    {
        ...
        try
        {
            ...
            Map<String, RemainTicketInfo> map = AtomLogic.getRemanentTicket(Constant.URL_4_REFERER_ORDER,
                    TicketQryCondition.getTestObject(), cookieHeaders);
            System.out.println(map);

            try
            {

                String[] tokens = doPreBookTicket(TicketQryCondition.getTestObject(), map.get("K1512"), cookieHeaders);
                ...
                doSubmitOrder(s, tokens, cookieHeaders, passengers, TicketQryCondition.getTestObject(),
                        map.get("K1512"));
            }
            catch (Exception e)
            {
